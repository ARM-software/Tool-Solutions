...
uint8_t inferenceResult [NUM_JOBS_PER_TASK][1001] __attribute__((section("output_data_sec"), aligned(4)));
...
void printResults(const char* name, uint32_t taskID)
{
    ...
    for (uint32_t i = 0; i < 1000; ++i) {
        if (topConfidence < inferenceResult[taskID][i]) {
            topConfidence = inferenceResult[taskID][i];
            topId = i;
        }
    }
