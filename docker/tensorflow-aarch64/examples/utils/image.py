...
def preprocess_image_for_classification(image_url, model_file):
    ...
    orig_image = tf.keras.preprocessing.image.load_img(image_file, target_size=dimensions)
...
def preprocess_image_for_detection(image_url, model_file):
    ...
    numpy_image = cv2.resize(numpy_image, tuple(dimensions), interpolation=cv2.INTER_LINEAR)

    # normalise image
    mean = np.array(model_descriptor['image_preprocess'][0]['mean'], dtype=np.float32)
    std = np.array(model_descriptor['image_preprocess'][0]['std'], dtype=np.float32)
    numpy_image = numpy_image / 255. - mean
...
def postprocess_image_for_detection(model_file, image_file, predictions, labels):
    '''
    ...
    for idx, score in enumerate(scores[0]):
        ...
        cv2.rectangle(image, (left, top), (left + label_width, top - label_height), (0, 255, 0), -1)
        cv2.putText(
            image,
            labels[idx],
            (left, top),
