...
def parse_arguments():
    ...
    assert args['runs'] > 0, "Number of inference runs must be greater then zero"
    assert args['threads'] > 0, "Number of threads must be greater then zero"

    # Check whether path to the model descriptor exists
    assert os.path.isfile(args['model']), "File describing model does not exists"
