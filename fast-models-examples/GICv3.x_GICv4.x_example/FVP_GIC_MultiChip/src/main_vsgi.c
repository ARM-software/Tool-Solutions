...
int main(void)
{
  ...
  setSGIBaseAddr(gic_its, gic_its_sgi);
  ...
  //itsVMAPP(0 /*vpeid*/, target_rd1, VCONFIG_TABLE, VPENDING_TABLE, 1 /*alloc*/, 1 /*v*/, 8192 /*doorbell*/, 14 /*size*/);
  itsVMAPP(gic_its, 0 /*vpeid*/, target_rd0, VCONFIG_TABLE, VPENDING_TABLE, 1 /*alloc*/, 1 /*v*/, 8192 /*doorbell*/, 14 /*size*/);
  itsINVDB(gic_its, 0 /*vpeid*/);
  itsSYNC(gic_its, target_rd0);

  /* DEBUG */
  // This command should not be required, but example fails without it :-S
  itsMAPC(gic_its, target_rd0, 0);
  /* END DEBUG */
  ...
  itsVSGI(gic_its, 0 /*vpeid*/, 0 /*vintid*/, 1 /*enable*/, 0 /*priority*/, 1 /*group*/, 0 /*clear*/);
  //itsMAPC(target_rd0, 0);
  ...
  //printf("main(): Sending vSGI 0 to vPEID 0, mapped to physical core 0.0.0.1\n");
  printf("main(): Sending vSGI 0 to vPEID 0, mapped to physical core 0.0.0.0\n");
  itsSendSGI(gic_its, 0 /*vintid*/, 0 /*vpeid*/);
...
uint32_t checkGICModel(struct GICv3AddressWrapper *gic, struct GICv3_its_ctlr_if* gic_its)
{
  ...
  //
  if (hasVSGI(gic_its, &gic->gic_rdist) == 0)
  {
