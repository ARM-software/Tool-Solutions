...
struct GICv3_dist_if
{
      ...
      const volatile uint32_t padding12[2048];        // +0xA000 - RESERVED

            // MultiChip Registers
            volatile uint32_t GICD_CHIPSR;            // +0xC000 - RW - Current Status of Chip in multiChip operations.
            volatile uint32_t GICD_DCHIPR;            // +0xC004 - RW - Specifies ID of Master Chip.
            volatile uint64_t GICD_CHIPR[16];         // +0xC008 - RW - Routing Table. Max num of chips is 16.
};
...
struct GICv3_its_ctlr_if
{
  ...
  const volatile uint32_t GITS_MPAMIDR;          // +0x0010 - RO - Reports maxmimum PARTID and PMG (GICv3.1)
        volatile uint32_t GITS_PARTIDR;          // +0x0014 - RW - Sets the PARTID and PMG used for ITS memory accesses (GICv3.1)
...
//             volatile uint64_t GICD_TYPER2;            // +0x000C - RESERVED
...
//   const volatile uint32_t GITS_MPIDR;            // +0x0018 - RO - ITS affinity, used for shared vPE table
//   const volatile uint32_t padding5;              // +0x001C - RESERVED
//         volatile uint64_t GITS_FCTLR;            // +0x0020 - RW - 
//         volatile uint64_t GITS_OPR;              // +0x0028 - RW -
//         volatile uint64_t GITS_OPSR;             // +0x0030 - RW -
//   const volatile uint32_t padding2[9];           // +0x0038 - RESERVED
//         volatile uint64_t GITS_CBASER;           // +0x0080 - RW - Sets base address of ITS command queue
//         volatile uint64_t GITS_CWRITER;          // +0x0088 - RW - Points to next enrty to add command
//         volatile uint64_t GITS_CREADR;           // +0x0090 - RW - Points to command being processed
//   const volatile uint32_t padding3[2];           // +0x0098 - RESERVED
